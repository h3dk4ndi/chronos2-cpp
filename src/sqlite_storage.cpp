...
// 18/08/2026 -- for adding new variables to the preprocessing table 
void SQLite::dropPrep() {
    const char* sql = "DROP TABLE IF EXISTS prep_data;";
    if (sqlite3_exec(db, sql, NULL, 0, &errMsg) != SQLITE_OK) {
        printf("dropPrep error: %s \n", errMsg); sqlite3_free(errMsg);
    }
};


// Preprocessed Data Handler
// 18/08/2026 - new variables coming!
void SQLite::createPrep() {
    const char* sql = R"(
        CREATE TABLE IF NOT EXISTS prep_data(
            ...
            rogersSatchell  REAL,
            yangZhang       REAL,
            negativeRealisedSemivar REAL,
            positiveRealisedSemivar REAL,
            bipowerVariation REAL, 
            signedJump      REAL, 
            leverage        REAL, 
            leverageMean5   REAL, 
            jumpComponent   REAL, 
...
void SQLite::insertPrep(const std::string& security, const PrepData& d) {
    if (sqlite3_prepare_v2(db,
            "INSERT OR REPLACE INTO prep_data "
            "(date, security, returns, close2closeRV, parkinson, garmanKlass, "
            "rogersSatchell, yangZhang, negativeRealisedSemivar, positiveRealisedSemivar, "
            "bipowerVariation, signedJump, leverage, leverageMean5, jumpComponent, "
            "relativeJump, modelTarget) "
    ...
    int col = 0;
    auto bind = [&](const std::vector<double>& v, size_t i) {
        long long j = (long long)i - (long long)(d.dates.size() - v.size());
    ...
    for (size_t i = 0; i < d.dates.size(); ++i) {
        col = 0;
        sqlite3_bind_int64(stmt, ++col, d.dates[i]);
        sqlite3_bind_text (stmt, ++col, security.c_str(), -1, SQLITE_TRANSIENT);
        bind(d.returns, i);                 bind(d.close2closeRV, i);
        bind(d.parkinson, i);               bind(d.garmanKlass, i);
        bind(d.rogersSatchell, i);          bind(d.yangZhang, i);
        bind(d.negativeRealisedSemivar, i); bind(d.positiveRealisedSemivar, i);
        bind(d.bipowerVariation, i);        bind(d.signedJump, i);
        bind(d.leverage, i);                bind(d.leverageMean5, i);
        bind(d.jumpComponent, i);           bind(d.relativeJump, i);
        bind(d.modelTarget, i);

        if (sqlite3_step(stmt) != SQLITE_DONE)
            printf("insertPrep failed at %lld: %s\n", (long long)d.dates[i], sqlite3_errmsg(db));
        sqlite3_reset(stmt);
    }
    sqlite3_finalize(stmt);
}
...
SQLite::PrepData SQLite::loadPrep(const std::string& sec) {
    ...
    const char* sql = R"(
        SELECT date, returns, close2closeRV, parkinson,
            garmanKlass, rogersSatchell, yangZhang, negativeRealisedSemivar, 
            positiveRealisedSemivar, bipowerVariation, signedJump, leverage, 
            leverageMean5, jumpComponent, relativeJump, modelTarget
        FROM prep_data
        WHERE security = ?
        ORDER BY date;
    )";
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        out.negativeRealisedSemivar.push_back(val(7));
        out.positiveRealisedSemivar.push_back(val(8));
        out.bipowerVariation   .push_back(val(9));
        out.signedJump         .push_back(val(10));
        out.leverage           .push_back(val(11)); 
        out.leverageMean5      .push_back(val(12)); 
        out.jumpComponent      .push_back(val(13)); 
        out.relativeJump       .push_back(val(14)); 
        out.modelTarget        .push_back(val(15));
    }
    sqlite3_finalize(stmt);
    return out;
}
